...
object Translator {
  /// Main method for translating specialized markup into html
  ...
  private def parse(line: String): String = {
    ...
    for( i <- 0 to (lineVector.length - 1) ) {
      
      if( !ignoreLine ) {
        
        var char = lineVector(i);
        ...
        // Otherwise check for inline syntax
          ...
          if( "%@_`" contains char ) {
            ...
            if( char == "%") { // Bold
              boldOpen match {
                case true => newline += "</b>";
                case false => newline += "<b>";
              }
              boldOpen = !boldOpen;
            } else if( char == "@" ) { // Italics
              italOpen match {
                case true => newline += "</i>";
                case false => newline += "<i>";
              }
              italOpen = !italOpen;
            } else if( char == "_") { // Underlined
              undrOpen match {
                case true => newline += "</u>";
                case false => newline += "<u>";
              }
              undrOpen = !undrOpen;
            } else if( char == "`") { // Code
              codeOpen match {
                case true => newline += "</code>";
                case false => newline += "<code>";
              }
              codeOpen = !codeOpen;
            }
  ...
  private def htmlHeadBoilerplate(file: MarkupFile, title: String): Unit = {
    ...
    <head>
        <meta charset="utf-8">""");
    // Break it up to squeeze the title & css style tags in
